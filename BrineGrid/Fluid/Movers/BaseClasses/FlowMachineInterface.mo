within BrineGrid.Fluid.Movers.BaseClasses;
model FlowMachineInterface
  "Partial model with performance curves for fans or pumps"
  extends Modelica.Blocks.Interfaces.BlockIcon;

  import cha = BrineGrid.Fluid.Movers.BaseClasses.Characteristics;

  parameter BrineGrid.Fluid.Movers.Data.Generic per
    "Record with performance data"
    annotation (choicesAllMatching=true,
      Placement(transformation(extent={{60,-80},{80,-60}})));

  parameter BrineGrid.Fluid.Movers.BaseClasses.Types.PrescribedVariable preVar=
    BrineGrid.Fluid.Movers.BaseClasses.Types.PrescribedVariable.Speed
...
protected
  final parameter Boolean preSpe=
    preVar == BrineGrid.Fluid.Movers.BaseClasses.Types.PrescribedVariable.Speed
    "True if speed is a prescribed variable of this block";
  final parameter Boolean prePre=
    preVar == BrineGrid.Fluid.Movers.BaseClasses.Types.PrescribedVariable.PressureDifference or
    preVar == BrineGrid.Fluid.Movers.BaseClasses.Types.PrescribedVariable.FlowRate
  ...
  final parameter
    BrineGrid.Fluid.Movers.BaseClasses.Characteristics.flowParametersInternal pCur1(
    final n = nOri,
  ...
  parameter
    BrineGrid.Fluid.Movers.BaseClasses.Characteristics.flowParametersInternal pCur2(
    final n = nOri + 1,
    ...
    "Volume flow rate vs. total pressure rise with correction for pump resistance added";
  parameter
    BrineGrid.Fluid.Movers.BaseClasses.Characteristics.flowParametersInternal pCur3(
    final n = nOri + 2,
    V_flow = if (haveVMax and haveDPMax) or (nOri == 2) then
               zeros(nOri + 2)
  ...
  parameter Real powDer[size(per.power.V_flow,1)]=
   if per.use_powerCharacteristic then
     BrineGrid.Utilities.Math.Functions.splineDerivatives(
                   x=per.power.V_flow,
                   y=per.power.P,
                   ensureMonotonicity=BrineGrid.Utilities.Math.Functions.isMonotonic(x=per.power.P,
                                                                                     strict=false))
   else
...
function getPerformanceDataAsString
  input BrineGrid.Fluid.Movers.BaseClasses.Characteristics.flowParameters pressure
      "Performance data";
...
initial equation
  ...
  assert(BrineGrid.Utilities.Math.Functions.isMonotonic(x=per.pressure.V_flow, strict=true) and
...
the simulation stops.");
  ...
  if (haveVMax and haveDPMax) or (nOri == 2) then  // ----- Curve 1
    ...
    preDer1= BrineGrid.Utilities.Math.Functions.splineDerivatives(x=pCur1.V_flow,
      y=pCur1.dp);
  ...
  elseif haveVMax or haveDPMax then  // ----- Curve 2
    ...
    preDer1= zeros(nOri);
    preDer2= BrineGrid.Utilities.Math.Functions.splineDerivatives(x=pCur2.V_flow,
      y=pCur2.dp);
    preDer3= zeros(nOri + 2);
  else  // ----- Curve 3
    ...
    preDer1= zeros(nOri);
    preDer2= zeros(nOri + 1);
    preDer3= BrineGrid.Utilities.Math.Functions.splineDerivatives(x=pCur3.V_flow,
      y=pCur3.dp);
  end if;
 ...
 motDer = if per.use_powerCharacteristic then zeros(size(per.motorEfficiency.V_flow,
    ...
    ensureMonotonicity=BrineGrid.Utilities.Math.Functions.isMonotonic(x=per.motorEfficiency.eta,
      strict=false));
  hydDer = if per.use_powerCharacteristic then zeros(size(per.hydraulicEfficiency.V_flow,
    1)) elseif (size(per.hydraulicEfficiency.V_flow, 1) == 1) then {0}
     else BrineGrid.Utilities.Math.Functions.splineDerivatives(x=per.hydraulicEfficiency.V_flow,
...
equation
  ...
  if (computePowerUsingSimilarityLaws == false) and preVar <> BrineGrid.Fluid.Movers.BaseClasses.Types.PrescribedVariable.Speed then
    r_N=1;
  else
  ...
  if per.use_powerCharacteristic then
    ...
    // a division by zero.
    eta = WFlo / BrineGrid.Utilities.Math.Functions.smoothMax(x1=PEle, x2=1E-5, deltaX=1E-6);
    ...
    // a division by zero.
    PEle = WFlo / BrineGrid.Utilities.Math.Functions.smoothMax(x1=eta, x2=1E-5, deltaX=1E-6);
    eta = etaHyd * etaMot;
...
<a href=\"modelica://BrineGrid.Fluid.Movers.Data.Generic\">
BrineGrid.Fluid.Movers.Data.Generic</a>.
