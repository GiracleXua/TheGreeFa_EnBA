...
model MixingVolume "Test model for mixing volumes"
  ...
  BrineGrid.Fluid.FixedResistances.PressureDrop res1(
    redeclare package Medium = Medium,
    from_dp=true,
    m_flow_nominal=2,
    dp_nominal=2.5)
  ...
  BrineGrid.Fluid.FixedResistances.PressureDrop res2(
    redeclare package Medium = Medium,
    from_dp=true,
    m_flow_nominal=2,
    dp_nominal=2.5)
  ...
  BrineGrid.Utilities.Diagnostics.CheckEquality cheEqu1
    "Check for equality of results"
    annotation (Placement(transformation(extent={{156,70},{176,90}})));
  BrineGrid.Fluid.Sensors.EnthalpyFlowRate entFloRat(
    redeclare package Medium = Medium,
    m_flow_nominal=2) "Enthalpy flow rate"
                                     annotation (Placement(transformation(
          extent={{40,50},{60,70}})));
  BrineGrid.Fluid.Sensors.EnthalpyFlowRate entFloRat1(
    redeclare package Medium = Medium,
    m_flow_nominal=2) "Enthalpy flow rate"
                                     annotation (Placement(transformation(
          extent={{40,0},{60,20}})));
  BrineGrid.Fluid.MixingVolumes.MixingVolumeMoistAir vol2(
    redeclare package Medium = Medium,
    V=0.1,
    nPorts=2,
    energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial,
  ...
  BrineGrid.Utilities.Diagnostics.CheckEquality cheEqu2
    "Check for equality of results"
    annotation (Placement(transformation(extent={{156,10},{176,30}})));
  BrineGrid.Fluid.Sensors.EnthalpyFlowRate entFloRat2(
    redeclare package Medium = Medium,
    m_flow_nominal=2) "Enthalpy flow rate"
...
__Dymola_Commands(file="modelica://BrineGrid/Resources/Scripts/Dymola/Fluid/MixingVolumes/Validation/MixingVolume.mos"
        "Simulate and plot"),
...
Standard Library with the implementation in the <code>BrineGrid</code>
library.
</p>
</html>", revisions="<html>
<ul>
