within BrineGrid.Utilities.Psychrometrics.Functions.Examples;
...
equation
  T = conv*time + 273.15;
  p_w_ashrae = BrineGrid.Utilities.Psychrometrics.Functions.pW_TDewPoi(T);
  p_w = BrineGrid.Utilities.Psychrometrics.Functions.pW_TDewPoi_amb(T);
  r_p = p_w_ashrae/p_w;
  p_w = BrineGrid.Utilities.Psychrometrics.Functions.pW_TDewPoi_amb(TInv);
  dT = T - TInv;
  assert(abs(dT) < 10E-12, "Error in function implementation.");
  annotation (
experiment(StopTime=1.0),
__Dymola_Commands(file="modelica://BrineGrid/Resources/Scripts/Dymola/Utilities/Psychrometrics/Functions/Examples/pW_TDewPoi_comparison.mos"
        "Simulate and plot"));
