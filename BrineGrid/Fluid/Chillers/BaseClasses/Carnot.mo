within BrineGrid.Fluid.Chillers.BaseClasses;
partial model Carnot
  ...
  Real etaPL(final unit = "1")=
    if evaluate_etaPL
      then 1
    else BrineGrid.Utilities.Math.Functions.polynomial(a=a, x=yPL)
    "Efficiency due to part load (etaPL(yPL=1)=1)";

  Real COP(min=0, final unit="1") = etaCarnot_nominal_internal * COPCar * etaPL
    "Coefficient of performance";

  Real COPCar(min=0) = TUse / BrineGrid.Utilities.Math.Functions.smoothMax(
    x1=1,
    x2=TCon-TEva,
...
initial equation
  ...
  assert(abs(BrineGrid.Utilities.Math.Functions.polynomial(
