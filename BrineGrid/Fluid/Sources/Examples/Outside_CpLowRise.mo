within BrineGrid.Fluid.Sources.Examples;
model Outside_CpLowRise
  "Test model for source and sink with outside weather data and wind pressure"
  extends Modelica.Icons.Example;
  package Medium = BrineGrid.Media.Air "Medium model for air";
  BrineGrid.Fluid.Sources.Outside_CpLowRise west(
  ...
  BrineGrid.Fluid.Sources.Outside_CpLowRise north(
    redeclare package Medium = Medium,
    s=1/5,
    azi=BrineGrid.Types.Azimuth.N,
    Cp0=0.6) "Model with outside conditions"
    annotation (Placement(transformation(extent={{-20,40},{0,60}})));
  BrineGrid.Fluid.Sources.Outside_CpLowRise south(
    redeclare package Medium = Medium,
    s=1/5,
    azi=BrineGrid.Types.Azimuth.S,
    Cp0=0.6) "Model with outside conditions"
    annotation (Placement(transformation(extent={{-20,-40},{0,-20}})));
  BrineGrid.Fluid.Sources.Outside_CpLowRise east(
    redeclare package Medium = Medium,
    s=5,
...
equation
  ...
  connect(weaDat.weaBus, east.weaBus) annotation (Line(
      ...
      thickness=0.5,
      smooth=Smooth.None));
  annotation (__Dymola_Commands(file="modelica://BrineGrid/Resources/Scripts/Dymola/Fluid/Sources/Examples/Outside_CpLowRise.mos"
        "Simulate and plot"),
