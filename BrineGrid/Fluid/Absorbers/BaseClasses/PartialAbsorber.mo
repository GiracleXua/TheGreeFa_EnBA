...
partial model PartialAbsorber
  ...
  //parameter Modelica.SIunits.MolarMass MM_Medium = Medium_b.MolarMass;
  parameter Real MM_Medium = Medium_b.MMX[2];
  ...
  parameter Modelica.SIunits.AbsolutePressure p_start_air=
    ...
    annotation (dialog(tab="Initialization",group="Pressure"));
  parameter Modelica.SIunits.Temperature T_start_abs=
  ...
  // Nominal
  parameter Modelica.SIunits.MassFlowRate m_flow_nominal_abs(min=0)
...
//---------Define heat and mass transfer component----------//
  replaceable model ThermalAir =
    ...
    "Model to calculate the gaseous heat transfer" annotation (Dialog(tab="Assumptions",
        group="Heat and mass transfer"), choicesAllMatching=true);
  replaceable model MoistureAir =
    ...
    "Model to calculate the gaseous mass transfer" annotation (Dialog(tab="Assumptions",
        group="Heat and mass transfer"), choicesAllMatching=true);
  replaceable model ThermalAbs =
      BrineGrid.HeatTransfer.Convection.Absorption.IdealHeatTransfer
    constrainedby BrineGrid.HeatTransfer.BaseClasses.PartialConvectionGeneral
    "Model to calculate the liquid heat transfer" annotation (Dialog(tab="Assumptions",
        group="Heat and mass transfer"), choicesAllMatching=true);
  replaceable model MoistureAbs =
      BrineGrid.MoistureTransfer.Convection.Absorption.IdealMassTransfer
    constrainedby
    BrineGrid.MoistureTransfer.BaseClasses.PartialConvectionGeneral
    "Model to calculate the liquid mass transfer" annotation (Dialog(tab="Assumptions",
        group="Heat and mass transfer"), choicesAllMatching=true);
//------End of Define heat and mass transfer component------//
  ...
  Modelica.Fluid.Interfaces.FluidPort_b port_b_out(
    ...
    annotation (Placement(transformation(extent={{50,-58},{70,-38}}),  iconTransformation(extent={{50,-58},
            {70,-38}})));
  ...
  Modelica.Fluid.Interfaces.FluidPort_b port_a_out(
    ...
    annotation (Placement(transformation(extent={{-70,-60},{-50,-40}}),  iconTransformation(extent={{-70,-60},
            {-50,-40}})));
protected
  ...
  // only valid for cross flow (discretization on the perpendicular direction)
...
equation
  ...
  //outport
  connect(floDis_air_out.port_a, port_a_out) annotation (Line(points={{-60,-20},{-60,-50}},
                                                      color={0,127,255}));
  connect(floDis_abs_out.port_a, port_b_out) annotation (Line(points={{60,-20},{60,-48}},
                               color={0,127,255}));
