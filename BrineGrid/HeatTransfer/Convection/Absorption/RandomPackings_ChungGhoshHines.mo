...
model RandomPackings_ChungGhoshHines
  "Random packings: Convective heat transfer correlation from Chung, Ghosh and Hines"
...
equation
  ...
  Pr = Fluid.Functions.CharacteristicNumbers.PrandtlNumber(
    ...
    cp=cp_a,
    lambda=lambda_a);
  Nu = 5.20e-5*X_b[1]^(1.56)*(m_flow_in_b/m_flow_in_a)^(0.50)*Pr^(0.333)*Re^(1.6);
  alpha = Nu*lambda_a/(a*deq^2);
