...
function power "Flow vs. electrical power characteristics for fan or pump"
  ...
  input BrineGrid.Fluid.Movers.BaseClasses.Characteristics.powerParameters per
...
algorithm
  if n == 1 then
    ...
    P:=r_N^3*BrineGrid.Utilities.Math.Functions.cubicHermiteLinearExtrapolation(
                x=rat,
                x1=per.V_flow[i],
                x2=per.V_flow[i + 1],
                y1=per.P[i],
