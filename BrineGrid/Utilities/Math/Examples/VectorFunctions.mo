...
model VectorFunctions "Test model for functions that take a vector as argument"
  ...
  BrineGrid.Utilities.Math.Min minVec(
                   nin=3)
  ...
  BrineGrid.Utilities.Math.Max maxVec(
                   nin=3)
    annotation (Placement(transformation(extent={{-20,-60},{0,-40}})));
  BrineGrid.Utilities.Math.Average aveVec(
                       nin=3)
...
__Dymola_Commands(file="modelica://BrineGrid/Resources/Scripts/Dymola/Utilities/Math/Examples/VectorFunctions.mos"
        "Simulate and plot"),
