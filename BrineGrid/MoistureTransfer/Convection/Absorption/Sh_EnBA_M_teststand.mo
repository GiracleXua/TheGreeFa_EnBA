...
equation
  ...
  Re = BrineGrid.Fluid.Functions.CharacteristicNumbers.ReynoldsNumber_m_flow(
    ...
    L=deq,
    A=crossArea);
  Sh = (param_sh[1])*(Re/2300)^(param_sh[2])*Sc^(param_sh[3])*(m_flow_in_b/m_flow_in_a)^(param_sh[4])*(x_eq/x-1)^(param_sh[5])*((T_b - 273.15)/(T_a - 273.15))^(param_sh[6]);
  beta =
