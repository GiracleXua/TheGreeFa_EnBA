 ...
 package Medium = BrineGrid.Media.Air
  ...
  Dampers.MixingBox mixBox(
    ...
    redeclare package Medium = Medium) "mixing box"
            ...
            -22},{34,-2}})));
    BrineGrid.Fluid.Sources.Boundary_pT bouIn(             redeclare package
      Medium = Medium, T=273.15 + 10,
    use_p_in=true,
    nPorts=2)                                             annotation (Placement(
        transformation(extent={{-60,2},{-40,22}})));
    BrineGrid.Fluid.Sources.Boundary_pT bouSup(             redeclare package
      Medium = Medium, T=273.15 + 26,
    use_p_in=true,
    nPorts=1)                                                                       annotation (Placement(
        transformation(extent={{68,-10},{48,10}})));
    BrineGrid.Fluid.Sources.Boundary_pT bouRet(             redeclare package
...
equation
  ...
  connect(bouRet.ports[1], mixBox.port_Ret) annotation (Line(
      ...
      color={0,127,255}));
  annotation (             __Dymola_Commands(file="modelica://BrineGrid/Resources/Scripts/Dymola/Fluid/Actuators/Dampers/Examples/MixingBox.mos"
        "Simulate and plot"),
