...
model SpeedControlled_y_linear
  ...
  BrineGrid.Fluid.Movers.SpeedControlled_y pumFixDp(
    redeclare package Medium = Medium,
  ...
  BrineGrid.Fluid.Sources.Boundary_pT sou1(
    ...
    nPorts=1) annotation (Placement(transformation(extent={{-10,-10},{10,10}},
          rotation=180,
        origin={128,90})));
  BrineGrid.Fluid.FixedResistances.PressureDrop dp1(
    redeclare package Medium = Medium,
  ...
  BrineGrid.Fluid.Movers.SpeedControlled_y pumFixM_flow(
    redeclare package Medium = Medium,
    energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState,
    per(pressure(V_flow=2/1000*{0, m_flow_nominal}, dp={2*dp_nominal, 0})),
    filteredSpeed=false) "Pump with fixed mass flow rate"
    annotation (Placement(transformation(extent={{40,40},{60,60}})));
  BrineGrid.Fluid.Sources.Boundary_pT sou3(
    redeclare package Medium = Medium,
    use_p_in=false,
    p(displayUnit="Pa") = 300000 + 0.01*dp_nominal,
    T=293.15,
...
__Dymola_Commands(file="modelica://BrineGrid/Resources/Scripts/Dymola/Fluid/Movers/Validation/SpeedControlled_y_linear.mos"
        "Simulate and plot"),
