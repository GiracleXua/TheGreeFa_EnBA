...
partial model PartialFlowMachine
  ...
  extends BrineGrid.Fluid.Interfaces.LumpedVolumeDeclarations(
    final mSenFac=1);
  extends BrineGrid.Fluid.Interfaces.PartialTwoPortInterface(
    show_T=false,
    port_a(
  ...
  replaceable parameter BrineGrid.Fluid.Movers.Data.Generic per
    constrainedby BrineGrid.Fluid.Movers.Data.Generic
    "Record with performance data"
    annotation (choicesAllMatching=true,
      Placement(transformation(extent={{52,60},{72,80}})));

  parameter BrineGrid.Fluid.Types.InputType inputType = BrineGrid.Fluid.Types.InputType.Continuous
    "Control input type"
    annotation(Dialog(
      group="Control"));
  parameter Real constInput = 0 "Constant input set point"
    annotation(Dialog(
      group="Control",
      enable=inputType == BrineGrid.Fluid.Types.InputType.Constant));
  parameter Real stageInputs[:]
    "Vector of input set points corresponding to stages"
    annotation(Dialog(
      group="Control",
      enable=inputType == BrineGrid.Fluid.Types.InputType.Stages));
  ...
  // Connectors and ports
    Modelica.Blocks.Interfaces.IntegerInput stage if inputType ==
    BrineGrid.Fluid.Types.InputType.Stages
...
protected
  ...
  // BrineGrid.Fluid.Movers.SpeedControlled_y$floMac1.
  ...
  Modelica.Blocks.Sources.Constant[size(stageInputs, 1)] stageValues(
    final k=stageInputs) if inputType == BrineGrid.Fluid.Types.InputType.Stages
                                                          "Stage input values"
    annotation (Placement(transformation(extent={{-80,40},{-60,60}})));
  Modelica.Blocks.Sources.Constant setConst(
    final k=constInput) if inputType == BrineGrid.Fluid.Types.InputType.Constant
    "Constant input set point"
    annotation (Placement(transformation(extent={{-80,70},{-60,90}})));

  Extractor extractor(final nin=size(stageInputs,1)) if inputType ==
    BrineGrid.Fluid.Types.InputType.Stages           "Stage input extractor"
  ...
  BrineGrid.Fluid.Delays.DelayFirstOrder vol(
  ...
  Modelica.Blocks.Math.Gain gaiSpe(y(final unit="1")) if inputType ==
    BrineGrid.Fluid.Types.InputType.Continuous and speedIsInput
    "Gain to normalized speed using speed_nominal or speed_rpm_nominal"
    annotation (Placement(transformation(extent={{-4,74},{-16,86}})));

  BrineGrid.Fluid.Movers.BaseClasses.IdealSource preSou(
    redeclare final package Medium = Medium,
    final m_flow_small=m_flow_small,
    final allowFlowReversal=allowFlowReversal,
    final control_m_flow= (preVar ==  BrineGrid.Fluid.Movers.BaseClasses.Types.PrescribedVariable.FlowRate))
    "Pressure source"
    annotation (Placement(transformation(extent={{40,-10},{60,10}})));

  BrineGrid.Fluid.Movers.BaseClasses.PowerInterface heaDis(
    final motorCooledByFluid=per.motorCooledByFluid,
    final delta_V_flow=1E-3*V_flow_max) if addPowerToMedium
  ...
  BrineGrid.Fluid.Sensors.MassFlowRate senMasFlo(
...
"*** Warning: You are using a flow or pressure controlled mover with the
  ...
  assert(nominalValuesDefineDefaultPressureCurve or
         (per.havePressureCurve or
           (preVar == BrineGrid.Fluid.Movers.BaseClasses.Types.PrescribedVariable.Speed)) or
...
<a href=\"modelica://BrineGrid.Fluid.Movers.Examples.MoverContinuous\">
BrineGrid.Fluid.Movers.Examples.MoverContinuous</a>.
</li>
<li>
