...
model OneEffectiveAirLeakageArea "Model with an effective air leakage area"
  ...
  Modelica.Blocks.Math.Gain Gain1(k=100) annotation (Placement(transformation(
          extent={{-40,20},{-20,40}})));
  BrineGrid.Airflow.Multizone.EffectiveAirLeakageArea cra(redeclare package
...
equation
  ...
  connect(Gain1.u, Sine1.y) annotation (Line(
      points={{-42,30},{-59,30}},
      color={0,0,127}));
  annotation (__Dymola_Commands(file="modelica://BrineGrid/Resources/Scripts/Dymola/Airflow/Multizone/Examples/OneEffectiveAirLeakageArea.mos"
        "Simulate and plot"),
        experiment(
      StopTime=7200,
      Tolerance=1e-05),
