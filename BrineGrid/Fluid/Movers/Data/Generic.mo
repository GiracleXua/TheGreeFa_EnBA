within BrineGrid.Fluid.Movers.Data;
record Generic "Generic data record for movers"
  ...
  parameter BrineGrid.Fluid.Movers.BaseClasses.Characteristics.flowParameters pressure(
    V_flow = {0, 0},
  ...
  parameter
    BrineGrid.Fluid.Movers.BaseClasses.Characteristics.efficiencyParameters
    hydraulicEfficiency(
      V_flow={0},
  ...
  parameter Real constantSpeed(final min=0, final unit="1") = constantSpeed_rpm/speed_rpm_nominal
    "Normalized speed set point, used if inputType = BrineGrid.Fluid.Types.InputType.Constant"
    annotation (Dialog(group="Normalized speeds (used in model, default values assigned from speeds in rpm"));

  parameter Real[:] speeds(each final min = 0, each final unit="1") = speeds_rpm/speed_rpm_nominal
    "Vector of normalized speed set points, used if inputType = BrineGrid.Fluid.Types.InputType.Stages"
    annotation (Dialog(group="Normalized speeds (used in model, default values assigned from speeds in rpm"));
  ...
  parameter Modelica.SIunits.Conversions.NonSIunits.AngularVelocity_rpm constantSpeed_rpm=speed_rpm_nominal
    "Speed set point, used if inputType = BrineGrid.Fluid.Types.InputType.Constant"
    annotation (Dialog(group="Speeds in RPM"));

  parameter Modelica.SIunits.Conversions.NonSIunits.AngularVelocity_rpm[:] speeds_rpm = {speed_rpm_nominal}
    "Vector of speed set points, used if inputType = BrineGrid.Fluid.Types.InputType.Stages"
    annotation (Dialog(group="Speeds in RPM"));
...
<pre>
  BrineGrid.Fluid.Movers.SpeedControlled_y fan(
...
BrineGrid.Fluid.Movers.SpeedControlled_y</a>,
...
<a href=\"modelica://BrineGrid.Fluid.Movers.Validation.Pump_Nrpm_stratos\">
BrineGrid.Fluid.Movers.Validation.Pump_Nrpm_stratos</a>.
</p>
