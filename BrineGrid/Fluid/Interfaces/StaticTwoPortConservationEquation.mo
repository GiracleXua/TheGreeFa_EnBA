within BrineGrid.Fluid.Interfaces;
...
equation
  ...
  // m_flow_inverse -- 1/m... so it need to be specially manipulated to avoid dividing by zero
  ...
  if use_m_flowInv then
    m_flowInv = BrineGrid.Utilities.Math.Functions.inverseXRegularized(
                       x=port_a.m_flow,
  ...
  if allowFlowReversal then
    ...
    hOut = BrineGrid.Utilities.Math.Functions.regStep(y1=port_b.h_outflow,
                                                    y2=port_a.h_outflow,
                                                    x=port_a.m_flow,
                                                    x_small=m_flow_small/1E3);
    XiOut = BrineGrid.Utilities.Math.Functions.regStep(y1=port_b.Xi_outflow,
                                                     y2=port_a.Xi_outflow,
                                                     x=port_a.m_flow,
                                                     x_small=m_flow_small/1E3);
    COut = BrineGrid.Utilities.Math.Functions.regStep(y1=port_b.C_outflow,
                                                    y2=port_a.C_outflow,
                                                    x=port_a.m_flow,
                                                    x_small=m_flow_small/1E3);
  else
  ...
  // Energy balance and mass balance
    ...
    if allowFlowReversal then
      ...
      port_a.Xi_outflow = Medium.X_default[1:Medium.nXi];//(Wang)no reversal means port_a nothing
    end if;
...
<a href=\"modelica://BrineGrid.Fluid.MassExchangers.Examples.ConstantEffectiveness\">
BrineGrid.Fluid.MassExchangers.Examples.ConstantEffectiveness</a>.
</li>
<li>
...
<a href=\"modelica://BrineGrid.Utilities.Math.Functions.inverseXRegularized\">
BrineGrid.Utilities.Math.Functions.inverseXRegularized</a>
