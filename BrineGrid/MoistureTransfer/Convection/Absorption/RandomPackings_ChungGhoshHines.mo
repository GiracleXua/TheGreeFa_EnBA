...
model RandomPackings_ChungGhoshHines
  "Random packings: Convective mass transfer correlation from Chung, Ghosh and Hines"
...
equation
  ...
  Sc = BrineGrid.Fluid.Functions.CharacteristicNumbers.SchmidtNumber(
    ...
    rho=rho_a,
    D=D_air);
  Sh = 1.326e-4*X_b[1]^(-0.94)*(m_flow_in_b/m_flow_in_a)^(0.27)*Sc^(0.333)*Re^(1.16);
  beta = Sh*D_air*rho_a/(BrineGrid.Media.LiquidDesiccants.Constants.MM_steam*a*deq^2);
...
end RandomPackings_ChungGhoshHines;
