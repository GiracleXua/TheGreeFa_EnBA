...
model PrescribedOutletState
  ...
  extends BrineGrid.Fluid.Interfaces.PartialTwoPortTransport(
    final dp_start=0,
    show_T=false,
    show_V_flow=false);
...
equation
  ...
  m_flow_pos = BrineGrid.Utilities.Math.Functions.smoothMax(
    x1=m_flow,
    x2=0,
    deltaX=m_flow_small);
  ...
  if not restrictHeat and not restrictCool then
    ...
    m_flow_limited = BrineGrid.Utilities.Math.Functions.smoothMax(
      x1 = port_a.m_flow,
      x2 = m_flow_small,
    ...
    if restrictHeat and restrictCool then
      ...
      dhSetAct = BrineGrid.Utilities.Math.Functions.smoothLimit(
    ...
    elseif restrictHeat then
      ...
      dhSetAct = BrineGrid.Utilities.Math.Functions.smoothMin(
        x1=hSet-inStream(port_a.h_outflow),
        x2=Q_flow_maxHeat / m_flow_limited,
        deltaX=deltah);
    else
      // Capacity limit for cooling only
      dhSetAct = BrineGrid.Utilities.Math.Functions.smoothMax(
        x1=hSet-inStream(port_a.h_outflow),
...
See <a href=\"modelica://BrineGrid.Fluid.HeatExchangers.HeaterCooler_T\">
BrineGrid.Fluid.HeatExchangers.HeaterCooler_T</a>
