...
class Calibration_H_M_transfer():
    ...
    def __init__(self,\
        ...
        max_iter = 10, delta_T =0.2, delta_x = 0.001, \
        header_of_input = '', \
        nNodes = 5, mNodes = 5,\
    ...
    def Nu_Sh_iteration(self, input_param, d_e, ex_area, \
        ...
        if np.isnan(input_param["T_d_o_exp"]):
            print('no exp data of desiccant outlet temperature available.')
            T_d_o_exp = T_d_in_exp
        else:
            T_d_o_exp = input_param["T_d_o_exp"]

        try:
            xi_d_o_exp = input_param['xi_d_o_exp']
        except:
            print('no exp data of desiccant outlet concentration available.')
            xi_d_o_exp = calc_xi_des_out(m_a=m_a_in_exp, m_des_in = m_d_in_exp, x_a_in = x_a_in_exp, x_a_out = x_a_out_exp, xi_des_in = xi_d_in_exp)            

        # estimation of Nu_0, Sh_0 according to exp_data
            # Nu_0
        Nu_0 = calc_Nu(d_e = d_e, m_a = m_a_in_exp, T_a_in = T_a_in_exp, T_a_out = T_a_out_exp, T_des_in = T_d_in_exp, T_des_out = T_d_o_exp, exchange_area = ex_area, flow = flow_config)
            # Sh_0
        Sh_0 = calc_Sh(d_e = d_e, m_a = m_a_in_exp, m_des_in = m_d_in_exp, x_a_in = x_a_in_exp, x_a_out = x_a_out_exp, T_des_in = T_d_in_exp, T_des_out = T_d_o_exp, xi_des_in = xi_d_in_exp, exchange_area = ex_area, T_a_in = T_a_in_exp, T_a_out = T_a_out_exp,  flow = flow_config)
        ...
        while True:
            ...
            x_a_out_sim_1 = air_mf_to_hr(result_1.values("abs.air_out.X[1]")[1][-1])
            T_d_out_sim_1 = result_1.values("abs.abs_out.T")[1][-1]
            xi_d_out_sim_1 = result_1.values("abs.abs_out.X[2]")[1][-1]
            
            print('T_a_out_sim_1, x_a_out_sim_1 are: {} and {}'.format(T_a_out_sim_1, x_a_out_sim_1))
            ...
            x_a_out_sim_2 = air_mf_to_hr(result_2.values("abs.air_out.X[1]")[1][-1])
            T_d_out_sim_2 = result_2.values("abs.abs_out.T")[1][-1]
            xi_d_out_sim_2 = result_2.values("abs.abs_out.X[2]")[1][-1]
            
            print('T_a_out_sim_2, x_a_out_sim_2 are: {} and {}'.format(T_a_out_sim_2, x_a_out_sim_2))

            delta_T_sim_exp_1 = T_a_out_sim_1 - T_a_out_exp
            delta_x_sim_exp_1 = x_a_out_sim_1 - x_a_out_exp
            
            delta_T_sim_exp_2 = T_a_out_sim_2 - T_a_out_exp
            delta_x_sim_exp_2 = x_a_out_sim_2 - x_a_out_exp
            ...
            if abs(delta_T_sim_exp_1) < abs(delta_T_sim_exp_2):
                ...
                T_d_out_sim_1, T_d_out_sim_2 = T_d_out_sim_2, T_d_out_sim_1
                xi_d_out_sim_1, xi_d_out_sim_2 = xi_d_out_sim_2, xi_d_out_sim_1
                
            if abs(delta_x_sim_exp_1) < abs(delta_x_sim_exp_2):
                Sh_1, Sh_2 = Sh_2, Sh_1
                x_a_out_sim_1, x_a_out_sim_2 = x_a_out_sim_2, x_a_out_sim_1
            
            list_T_a.append(T_a_out_sim_2)
            list_x_a.append(x_a_out_sim_2)
            list_T_d.append(T_d_out_sim_2)
            list_xi_d.append(xi_d_out_sim_2)
        ...
        df_export = pd.DataFrame(zip(list_Nu_1, list_Nu_2, list_Sh_1, list_Sh_2, list_T_a, list_x_a), columns=['Nu_1', 'Nu_2', 'Sh_1', 'Sh_2', 'T_a_o(K)', 'x_a_o(kg/kg)'])
