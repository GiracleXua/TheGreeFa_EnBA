...
equation
  X = conv*time;
  p_w = BrineGrid.Utilities.Psychrometrics.Functions.pW_X(X);
  XInv = BrineGrid.Utilities.Psychrometrics.Functions.X_pW(p_w);
  dX = X - XInv;
  assert(abs(dX) < 10E-12, "Error in function implementation.");
  annotation (
experiment(StopTime=1.0),
__Dymola_Commands(file="modelica://BrineGrid/Resources/Scripts/Dymola/Utilities/Psychrometrics/Functions/Examples/pW_X.mos"
        "Simulate and plot"));
end pW_X;
