within BrineGrid.Fluid.Actuators.Valves.Examples;
model ValveParameterization
 ...
 package Medium = BrineGrid.Media.Water;
  ...
  BrineGrid.Fluid.Actuators.Valves.TwoWayLinear valOPPoi(
    redeclare package Medium = Medium,
    m_flow_nominal=150/3600,
    CvData=BrineGrid.Fluid.Types.CvTypes.OpPoint,
  ...
  BrineGrid.Fluid.Sources.Boundary_pT sou(             redeclare package Medium =
        Medium,
    use_p_in=true,
    nPorts=3,
    T=293.15) "Boundary condition for flow source"  annotation (Placement(
        transformation(extent={{-70,-10},{-50,10}})));
  BrineGrid.Fluid.Sources.Boundary_pT sin(             redeclare package Medium =
        Medium,
    nPorts=3,
    use_p_in=false,
    p=300000,
  ...
  Valves.TwoWayLinear valKv(
    redeclare package Medium = Medium,
    CvData=BrineGrid.Fluid.Types.CvTypes.Kv,
    m_flow_nominal=150/3600,
    Kv=0.73,
    filteredOpening=false) "Valve model, linear opening characteristics"
  ...
  Valves.TwoWayLinear valCv(
    redeclare package Medium = Medium,
    m_flow_nominal=150/3600,
    CvData=BrineGrid.Fluid.Types.CvTypes.Cv,
    Cv=0.84,
    filteredOpening=false) "Valve model, linear opening characteristics"
         annotation (Placement(transformation(extent={{-10,-50},{10,-30}})));
  BrineGrid.Fluid.Sensors.MassFlowRate senM_flowOpPoi(redeclare package Medium =
        Medium) annotation (Placement(transformation(extent={{20,30},{40,50}})));
  BrineGrid.Fluid.Sensors.MassFlowRate senM_flowKv(redeclare package Medium =
        Medium) annotation (Placement(transformation(extent={{20,-10},{40,10}})));
  BrineGrid.Fluid.Sensors.MassFlowRate senM_flowCv(redeclare package Medium =
        Medium)
    annotation (Placement(transformation(extent={{20,-50},{40,-30}})));
  BrineGrid.Utilities.Diagnostics.AssertEquality equ1(threShold=0.01)
    annotation (Placement(transformation(extent={{80,60},{100,80}})));
  BrineGrid.Utilities.Diagnostics.AssertEquality equ2(threShold=0.01)
...
__Dymola_Commands(file="modelica://BrineGrid/Resources/Scripts/Dymola/Fluid/Actuators/Valves/Examples/ValveParameterization.mos"
        "Simulate and plot"),
