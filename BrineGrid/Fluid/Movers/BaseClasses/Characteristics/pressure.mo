within BrineGrid.Fluid.Movers.BaseClasses.Characteristics;
function pressure
  ...
  input BrineGrid.Fluid.Movers.BaseClasses.Characteristics.flowParametersInternal per
...
algorithm
  ...
  if r_N>=0 then
    dp:=r_N^2*BrineGrid.Utilities.Math.Functions.cubicHermiteLinearExtrapolation(
              x=rat,
              x1=per.V_flow[i],
              x2=per.V_flow[i + 1],
              y1=per.dp[i],
...
is now called by
<a href=\"modelica://BrineGrid.Fluid.Movers.BaseClasses.FlowMachineInterface\">
BrineGrid.Fluid.Movers.BaseClasses.FlowMachineInterface</a>.
</li>
</ul>
</html>"));
end pressure;
