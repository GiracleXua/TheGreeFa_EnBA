...
function efficiency "Flow vs. efficiency characteristics for fan or pump"
  ...
  input BrineGrid.Fluid.Movers.BaseClasses.Characteristics.efficiencyParameters per
...
algorithm
  if n == 1 then
    ...
    rat:=V_flow/
            BrineGrid.Utilities.Math.Functions.smoothMax(
              x1=r_N,
              x2=0.1,
              deltaX=delta);
    i :=1;
    ...
    eta:=BrineGrid.Utilities.Math.Functions.cubicHermiteLinearExtrapolation(
                x=rat,
                x1=per.V_flow[i],
                x2=per.V_flow[i + 1],
                y1=per.eta[i],
