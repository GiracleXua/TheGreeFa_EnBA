...
package LithiumBromideAqueousSolution_PatekKlomfar "H2O-LiBr: Lithium bromide aqueous solution (273 K ... 500 K, 0 wt-% ... 75 wt-%)"
  extends BrineGrid.Media.Examples.PartialGeneralBrineMedia(
    ...
    final MMX={Modelica.Media.IdealGases.Common.SingleGasesData.H2O.MM, Modelica.Media.IdealGases.Common.SingleGasesData.LiBr.MM},
    final RX={Modelica.Media.IdealGases.Common.SingleGasesData.H2O.R, Modelica.Media.IdealGases.Common.SingleGasesData.LiBr.R},
...
protected
    final constant Real[3] a={-0.14511,16.3967,-494.122};
    final constant Real[3] b={8.52755,-934.568,28606.4};
    final constant Real[3] c={0.0207809,-2.35014,70.3848};
    Real poly1;
    Real poly2;
    Real poly3;
  algorithm
    ...
    for i in 2:size(a, 1) loop
      poly1 := poly1*(state.X[Desiccant]*100) + a[i];
      poly2 := poly2*(state.X[Desiccant]*100) + b[i];
      poly3 := poly3*(state.X[Desiccant]*100) + c[i];
    end for;
    eta := exp(poly1 + poly2/state.T + poly3*log(state.T))/1000;
...
<p>This model allows the calculation of fluid properties of lithium bromide
aqueous solution (LiBr-H2O) in the fluid region of 273 K to 500 K and from
0 wt-% to 75 wt-% LiCl. </p>
...
very accurate media models are needed. This medium model is based on the thermal, caloric
...
<dd><b>A computationally effective formulation of the thermodynamic properties of LiBr-H2O solutions from 273 to 500 K over full composition range</b><br>
International Journal of Refrigeration 29, p. 566-578 (2006)<br>
DOI: <a href=\"http://dx.doi.org/10.1016/j.ijrefrig.2005.10.007\">10.1016/j.ijrefrig.2005.10.007</a>
</dd></dl>
</html>",     revisions="<html>
<ul>
<li>
...
end LithiumBromideAqueousSolution_PatekKlomfar;
