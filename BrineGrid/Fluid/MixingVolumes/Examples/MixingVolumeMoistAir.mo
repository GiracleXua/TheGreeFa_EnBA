within BrineGrid.Fluid.MixingVolumes.Examples;
model MixingVolumeMoistAir "Test model for mixing volume with moist air input"
  ...
  BrineGrid.Fluid.MixingVolumes.MixingVolumeMoistAir vol1(
  ...
  Modelica.Blocks.Sources.Constant TSet(k=273.15 + 20)
    "Set point for temperature" annotation (Placement(transformation(extent={{
            -80,120},{-60,140}})));
  BrineGrid.Utilities.Psychrometrics.pW_X humRat(           use_p_in=false)
    "Conversion from humidity ratio to partial water vapor pressure"
    annotation (Placement(transformation(extent={{-20,-120},{0,-100}})));
  BrineGrid.Utilities.Psychrometrics.TDewPoi_pW dewPoi "Dew point temperature"
  ...
  Modelica.Blocks.Sources.RealExpression QLat_flow(y=vol1.QLat_flow.y)
    "MoistAir heat flow rate" annotation (Placement(transformation(extent={{112,
            60},{132,80}})));
  BrineGrid.Fluid.Sources.MassFlowSource_T sou(
    redeclare package Medium = Medium,
    nPorts=1,
  ...
  BrineGrid.Fluid.Sources.Boundary_pT sin(        redeclare package Medium =
    ...
    nPorts=1)             annotation (Placement(transformation(
        ...
        extent={{-10,-10},{10,10}},
        rotation=180)));
  BrineGrid.Controls.Continuous.LimPID PI(
    Ni=0.1,
    yMax=1000,
    k=1,
    Ti=1,
  ...
  BrineGrid.Controls.Continuous.LimPID PI1(
    Ni=0.1,
    Ti=1,
    controllerType=Modelica.Blocks.Types.SimpleController.PI,
    k=10,
  ...
  BrineGrid.Fluid.Sensors.MassFlowRate mIn_flow(redeclare package Medium =
        Medium) annotation (Placement(transformation(extent={{6,-10},{26,10}})));
  BrineGrid.Fluid.Sensors.MassFlowRate mOut_flow(redeclare package Medium =
...
experiment(StopTime=600,
           Tolerance=1e-6),
__Dymola_Commands(file="modelica://BrineGrid/Resources/Scripts/Dymola/Fluid/MixingVolumes/Examples/MixingVolumeMoistAir.mos"
        "Simulate and plot"),
    Documentation(info="<html>
<p>
This model tests
<a href=\"modelica://BrineGrid.Fluid.MixingVolumes.MixingVolumeMoistAir\">
BrineGrid.Fluid.MixingVolumes.MixingVolumeMoistAir</a>.
